...
typedef struct _PluginVersion
{
  ...
  const char *pluginname;  //make this point to a 0-terminated string (allocated memory or static addressin your dll, not stack)
...
typedef struct _PLUGINTYPE5_INIT
{
  ...
  const char* shortcut; //0 terminated string containing the shortcut in textform. CE will try it's best to parse it to a valid shortcut
